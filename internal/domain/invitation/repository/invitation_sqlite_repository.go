...
type SQLiteInvitationRow struct {
	ID            string
...
var NoSQLiteInvitationRow = SQLiteInvitationRow{}

func NewInvitationSQLiteRepository(logger logger.Logger, sqldbm dhasar.SQLDatabaseManager) (InvitationRepository, error) {
	return dhasar.NewSQLiteRepository(dhasar.SQLiteRepositoryOption[
		...
		Scan: func(rows *sql.Rows) (SQLiteInvitationRow, error) {
			row := SQLiteInvitationRow{}
			...
			if err := rows.Scan(&row.ID, &row.RecipientName, &row.Code, &row.Status, &row.CreatedAt, &row.UpdatedAt, &row.OpenedAt, &row.CancelledAt); err != nil {
				return NoSQLiteInvitationRow, err
			}

			return row, nil
		},
		Entity: func(row SQLiteInvitationRow) invitation_entity.Invitation {
		...
		Row: func(invitation invitation_entity.Invitation) SQLiteInvitationRow {
			return SQLiteInvitationRow{
		...
		Values: func(row SQLiteInvitationRow) []any {
			return []any{
				row.ID,
				row.RecipientName,
				row.Code,
